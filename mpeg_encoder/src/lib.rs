...
impl Encoder {
    ...
    fn encode(&mut self,
        ...
        unsafe {
            self.scale_context =
                ffmpeg_sys_next::sws_getCachedContext(self.scale_context,
                                                 width as i32,
                                                 height as i32,
                                                 AVPixelFormat::AV_PIX_FMT_RGB24,
        ...
        unsafe {
            ret = ffmpeg_sys_next::avcodec_encode_video2(self.context,
                                                    &mut pkt,
                                                    self.frame,
                                                    &mut got_output);
        }
        ...
        unsafe {
            (*self.frame).pts +=
                ffmpeg_sys_next::av_rescale_q(1, (*self.context).time_base, (*self.video_st).time_base);
    ...
    pub fn init(&mut self) {
        ...
        unsafe {
            ...
            /*
            if (*fmt).flags & ffmpeg_sys_next::AVFMT_GLOBALHEADER != 0 {
            ...
            */
            ...
            // alloc the buffer
            let nframe_bytes = ffmpeg_sys_next::avpicture_get_size(self.pix_fmt,
                                                              self.target_width as i32,
                                                              self.target_height as i32);
            ...
            let _ = ffmpeg_sys_next::avpicture_fill(self.frame as *mut AVPicture,
            ...
            if ffmpeg_sys_next::avcodec_parameters_from_context((*self.video_st).codecpar, self.context)
                < 0
            {
    ...
    fn drop(&mut self) {
        if self.initialized {
            ...
            while got_output != 0 {
                ...
                if got_output != 0 {
                    unsafe {
                        let _ = ffmpeg_sys_next::av_interleaved_write_frame(self.format_context,
                                                                       &mut pkt);
                        ffmpeg_sys_next::av_free_packet(&mut pkt);
                    }
                }
